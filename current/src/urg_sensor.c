...
static int receive_data(urg_t *urg, long data[], long* io, unsigned short intensity[],
                        long *time_stamp)
{
    ...
    if (urg->specified_scan_times != 1) {
        if (!strncmp(buffer, "00", 2)) {
            ...
            if (n != 0) {
                ...
                return receive_data(urg, data, io, intensity, time_stamp);
            }
        }
    }
    ...
    case URG_DISTANCE:
    case URG_MULTIECHO:
        ret = receive_length_data(urg, data, NULL, NULL, type, buffer);
        break;

    case URG_DISTANCE_INTENSITY:
    case URG_MULTIECHO_INTENSITY:
        ret = receive_length_data(urg, data, NULL, intensity, type, buffer);
        break;
    case URG_DISTANCE_IO:
        ret = receive_length_data(urg, data, io, NULL, type, buffer);
        break;
    case URG_DISTANCE_INTENSITY_IO:
        ret = receive_length_data(urg, data, io, intensity, type, buffer);
        break;
    case URG_STOP:
    case URG_UNKNOWN:
        ret = 0;
...
int urg_get_distance_io(urg_t* urg, long data[], long* io, long* time_stamp)
{
    ...
    return receive_data(urg, data, io, NULL, time_stamp);
}
                               ...
                               long *time_stamp)
{
    ...
    return receive_data(urg, data, NULL, intensity, time_stamp);
}

int urg_get_distance_intensity_io(urg_t* urg,
    long data[], long* io, unsigned short intensity[],
    long* time_stamp)
{
    if (!urg->is_active) {
        return set_errno_and_return(urg, URG_NOT_CONNECTED);
                                ...
                                long *time_stamp)
{
    ...
    return receive_data(urg, data_multi, NULL, intensity_multi, time_stamp);
}
...
int urg_is_stable(urg_t *urg)
{
    ...
    return (strncmp("Stable 000 no error", stat, 19) == 0 || strncmp("Sensor works well", stat, 17) == 0
        ...
        strncmp("failed to synchronize", stat, 21) == 0 || strncmp("detected multiple master sensor", stat, 31) == 0) ? 1: 0;
}
